...
- **下班倒计时** — 弹性工作制：早上首次开屏记为上班，下班点 = 上班 + 9.5h，进度百分比 + 最后半小时高亮，过点显示加班时长
...
```
09:30 开屏 → 🕔 下班 8h30m·10% (09:30→19:00)
10:12 开屏 → 🕔 下班 9h12m·3%  (10:12→19:42)
11:40 开屏 → 🕔 下班 3h08m·67% (11:40→21:10)
```

打卡记录在 `~/.ccday-punch.json`，**只对当天有效**，隔夜不沿用。
记错了就 `rm ~/.ccday-punch.json`，下次刷新（仍在窗口内）重新打卡。

**退回固定时间**的三种情况——都用下面这组配置：

- `CCDAY_WORK_PUNCH=0` 手动关闭打卡
- 首次开屏晚于 `CCDAY_PUNCH_END`（下午才开电脑，没法推断上班时间）
...
| 工作中 | `🕔 下班 3h45m·60% (10:12→19:42)` | 剩余时长 + 当日进度（括号仅打卡模式） |
...
CCDAY_WORK_PUNCH=1                    # 1=按首次开屏打卡（默认），0=用固定时间
...
| `~/.ccday-punch.json` | 当天上班打卡时间（首次开屏），隔夜自动作废 |
...
- **v0.6.5** — 🕔 下班倒计时改为弹性工作制：早上 `CCDAY_PUNCH_START`–`CCDAY_PUNCH_END`（默认 06:00–12:00）间首次开屏记为上班时间，下班点 = 上班 + `CCDAY_WORK_HOURS`（默认 9.5h），倒计时带上 `(10:12→19:42)`。打卡记录只对当天有效，隔夜作废；首次开屏晚于窗口、跨天班、`CCDAY_WORK_PUNCH=0` 时退回固定 `WORK_START/END`。🏖 周末倒计时同步跟随打卡下班点
