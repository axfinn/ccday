...
1. **macOS** — `pmset -g log` 里当天窗口内第一次真实用户活动：显示器点亮、
   ...
   —— 机器自己醒来收邮件不等于人到了
2. **macOS 但 `pmset` 一条活动都查不到** — 看 `ioreg` 里的键鼠空闲时间，只有 2 分钟内
   动过键鼠（人此刻真在机器前）才拿当前时间打卡；空闲就先不打，等人来
3. **其他平台** — 没有空闲信号可用，退回状态栏在窗口内的首次刷新时刻

**状态栏刷新不等于人在。** 机器整夜没关、挂着会话空转时状态栏照样会刷，那一刷不能算上班
（否则上班时间会被记成 06:00）。所以 `pmset` 和键鼠都表明人不在时，宁可先不打卡。

`pmset` 查到真实活动后即认定，不再重复拉日志；查不到则每 5 分钟复查一次——早上 6 点日志里
还没有任何活动，人 10 点才到，一次查不到就放弃会把上班时间永远钉在兜底值上。兜底值（键鼠/
首刷）都只是暂定，之后每次复查都给 `pmset` 纠正的机会。
...
打卡记录在 `~/.ccday-punch.json`，**只对当天有效**，隔夜不沿用。`source` 字段标明来源：

| `source` | 含义 |
|---|---|
| `pmset` | 日志里的真实首次活动，权威值，认定后不再改 |
| `hid` | `pmset` 查不到，但键鼠显示人在，取当时时间（仍可被 `pmset` 纠正） |
| `refresh` | 非 macOS，没有空闲信号可用时的状态栏首刷兜底 |
| `idle` | 窗口内确认还没人动过机器，尚未打卡 |
| `closed` | 打卡窗口已关，就地冻结不再重试 |

记错了就 `rm ~/.ccday-punch.json`，下次刷新（仍在窗口内且人在操作）重新打卡。
...
- **v0.6.7** — 🕔 修上班时间被记成清早的问题。机器整夜没关时（后台断言阻止休眠），清早状态栏刷一次就走 `refresh` 兜底把上班时间钉在 06:00，下班点变成 15:00；那会儿 `pmset` 日志里还没有任何用户活动，而一旦落盘，`load_punch()` 命中缓存就直接返回，真人 10:39 到场也再没回头查过。三处改动：① `pmset` 查不到时按 5 分钟节流复查，不再一次失败就永久放弃，兜底值随后可被真实值纠正；② macOS 新增键鼠兜底——读 `ioreg` 的 `HIDIdleTime`，只有 2 分钟内动过键鼠才拿当前时间打卡，空闲则记 `idle` 等人来，状态栏刷新不再被当成「人在」；③ 打卡窗口关闭后就地冻结为 `closed`，有暂定值保留、没有则退回固定 `WORK_START/END`。`source` 扩展为 `pmset`/`hid`/`refresh`/`idle`/`closed` 五种，`pmset` 为权威值不再改写；手改打卡时间需写 `source: pmset` 才不会被自动探测覆盖。
